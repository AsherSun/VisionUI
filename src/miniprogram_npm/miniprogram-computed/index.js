...
module.exports = Behavior({
  ...
  definitionFilter: function definitionFilter(defFields) {
    // defFields 参数指 组件的本身
    var computed = defFields.computed || {}; // 拿到computed对象
    var computedKeys = Object.keys(computed); // 拿到computed对象下的key集合
    var computedCache = {}; // Cache
    var doingSetData = false; // 是否正在更新数据

    // 计算 computed
    var calcComputed = function calcComputed(scope, insertToData) {
      var needUpdate = {}; // 定义一个需要更新的对象
